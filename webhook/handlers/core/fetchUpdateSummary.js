...
const handler = (interaction) => {
  return new Promise((resolve, reject) => {
    ...
    axios
      .get(`https://crnzwhack.herokuapp.com/updateSummary`)
      .then(axiosResponse => {
        // Retrieve the prices from the response object, in USD and EUR
        let updateSummary = axiosResponse.data

        // Check if the API returned an error
        if (updateSummary.Response && updateSummary.Response === 'Error') {
          // The API returned an error
          // So build the response object to trigger the failure intent
          interaction.response.followupEvent = {
            name: 'updateSummary-not-found',
            data: {}
          }
        } else {
